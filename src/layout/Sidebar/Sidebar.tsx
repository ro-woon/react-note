...
const Sidebar = () => {
  ...
  const { isOpen } = useAppSelector((state) => state.menu); // redux에서 state 가져오기
  const { tagsList } = useAppSelector((state) => state.tags); // redux에서 state 가져오기

  const { pathname } = useLocation(); // 현재 url 받아오기
