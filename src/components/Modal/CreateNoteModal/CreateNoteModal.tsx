...
const CreateNoteModal = () => {
  const dispatch = useAppDispatch(); //redux용 dispatch

  const { editNote } = useAppSelector((state) => state.notesList); // redux에서 state 가져오기
  const { viewAddTagsModal } = useAppSelector((state) => state.modal);// redux에서 state 가져오기

   // useState로 여기서만 쓰이는 상태 관리, 굳이 다른 컴포넌트에서 안 쓰이는 상태를 관리할 필요가 없음
  ...
  // redux를 통해 createModal을 닫기 위해 dispatch
